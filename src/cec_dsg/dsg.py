...
class DSG:
    ...
    def __find_closest_match__(self, object_class:str, x:float, y:float, z:float, ignore:list=[]):
        # check if the object class is known
        if object_class not in self.__objects_by_class__:
            raise KeyError("The given object class '" + str(object_class) + "' is not in the known objects list.")
    ...
    def __validate_object_properties__(self, object_dict:Optional[dict]=None, object_class:Optional[str]=None, x:Optional[float]=None, y:Optional[float]=None, z:Optional[float]=None):
